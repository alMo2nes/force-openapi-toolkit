...
export default class OpenApiConfigManager extends LightningElement {
  ...
  loadCodeMirror() {
    ...
    this.codeMirrorLoadingPromise = Promise.all([
      loadStyle(this, base + "/lib/codemirror.css"),
      loadStyle(this, base + "/addon/lint/lint.css"),
      loadStyle(this, base + "/addon/hint/show-hint.css"),
      loadStyle(this, base + "/addon/fold/foldgutter.css"),
      loadStyle(this, base + "/theme/dracula.css"),
      loadStyle(this, base + "/theme/material.css"),
      loadStyle(this, base + "/theme/monokai.css"),
      loadScript(this, base + "/lib/codemirror.js"),
      loadScript(this, base + "/jsonlint.js")
    ])
      .then(() => {
        return Promise.all([
          loadScript(this, base + "/mode/javascript/javascript.js"),
          loadScript(this, base + "/addon/lint/lint.js"),
          loadScript(this, base + "/addon/edit/matchbrackets.js"),
          loadScript(this, base + "/addon/edit/closebrackets.js"),
          loadScript(this, base + "/addon/hint/show-hint.js"),
          loadScript(this, base + "/addon/fold/foldcode.js"),
          loadScript(this, base + "/addon/fold/foldgutter.js"),
          loadScript(this, base + "/addon/fold/brace-fold.js"),
          loadScript(this, base + "/addon/hint/anyword-hint.js"),
          loadScript(this, base + "/addon/lint/json-lint.js")
        ]);
      })
      .then(() => {
  ...
  registerOpenApiHinter() {
    ...
    // Register under "javascript" so it matches the editor mode used for JSON editing.
    window.CodeMirror.registerHelper("hint", "javascript", (editor) => {
      const cur = editor.getCursor();
      const token = editor.getTokenAt(cur);
      // Determine the prefix to filter keyword suggestions.
      let search = token.string.replace(/^"|"$/g, "").trim();
      if (token.type === null && token.string === "") {
        search = "";
      }
      const list = OPENAPI_KEYWORDS.filter((w) =>
        w.toLowerCase().startsWith(search.toLowerCase())
      ).map((w) => ({ text: '"' + w + '": ', displayText: w }));

      // Fall back to returning an empty list rather than undefined so CodeMirror
      // doesn't throw when no keywords match.
      return {
  ...
  async loadConfigurations(options = {}) {
    ...
    try {
      ...
      if (
        retryWhenEmpty &&
        data.length === 0 &&
        emptyRetryCount < MAX_EMPTY_RELOAD_RETRIES
      ) {
  ...
  initEditors() {
    if (!this.codeMirrorLoaded || !this.comparisons || this.editorsInitialized)
      return;

    this.comparisons.forEach((comp) => {
      const workingHost = this.template.querySelector(
        `.working-editor[data-key="${comp.methodName}"]`
      );
      ...
      const config = {
        ...
        readOnly,
        // Wire up Ctrl+Space to trigger autocomplete manually.
        extraKeys: readOnly ? {} : { "Ctrl-Space": "autocomplete" },
        // Automatically show hints after every keystroke in editable editors.
        hintOptions: readOnly
          ? undefined
          : {
              hint: window.CodeMirror.hint.javascript,
              completeSingle: false
            }
      };

      const ed = window.CodeMirror(workingHost, config);
      ed.setValue(comp.currentSpec || "");
      ed.setSize(null, 500);
      if (!readOnly) {
        ed.on("change", (cm) => {
          this.updateComparisonField(
            comp.methodName,
            "currentSpec",
            cm.getValue()
          );
          // Trigger autocomplete on every keystroke (suppress when already shown).
          if (!cm.state.completionActive) {
            window.CodeMirror.commands.autocomplete(cm, null, {
              completeSingle: false
            });
          }
        });
    ...
    this.editorsInitialized =
      ...
      this.comparisons.every((comp) =>
        this.editors.has(comp.methodName + "_working")
      );
  ...
  initModalEditor() {
    if (
      !this.isSourceSpecModalOpen ||
      !this.codeMirrorLoaded ||
      this.modalEditorInitialized
    )
      return;
    ...
    if (modalHost) {
      ...
      const config = {
        ...
        indentUnit: 2,
        theme: this.selectedTheme,
        // Modal is always read-only; no autocomplete needed.
        readOnly: true
      };
  ...
  async handleSave() {
    if (this.isSaving || !this.comparisons || this.comparisons.length === 0)
      return;
    ...
    try {
      ...
      this.showToast(
        "Deploying",
        "Deployment started. Monitoring status...",
        "info"
      );
      ...
      try {
        ...
        if (TERMINAL_DEPLOYMENT_STATUSES.includes(status)) {
          if (SUCCESS_DEPLOYMENT_STATUSES.includes(status)) {
            ...
            this.isSaving = false;
            publish(this.messageContext, API_STUDIO_EVENTS, {
  ...
  handleThemeChange(event) {
    ...
    this.editors.forEach((editor) =>
      editor.setOption("theme", this.selectedTheme)
    );
    if (this.modalEditor) {
  ...
  handleValidate(event) {
    const ed = this.editors.get(event.target.dataset.key + "_working");
    try {
      JSON.parse(ed ? ed.getValue() : "");
      this.showToast("Valid", "JSON is correct.", "success");
    } catch (e) {
